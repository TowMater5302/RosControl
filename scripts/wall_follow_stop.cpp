...
WallFollow::WallFollow(ros::NodeHandle* nh) {
    ...
    nh->getParam("force_turn_at", this->force_turn_at);
    ROS_INFO("Set force_turn_at: %f", this->force_turn_at);

    nh->getParam("force_turn_strength", this->force_turn_strength);
    ROS_INFO("Set force_turn_strength: %f", this->force_turn_strength);
    ...
    nh->getParam("center_wall_detect_d", this->center_wall_detect_d);
    ROS_INFO("Set center_wall_detect_d: %f", this->center_wall_detect_d);
...
void WallFollow::handle_pid(){
    ...
    cv::Mat center_wall = this->depth_array(cv::Range(200, center_v), cv::Range(center_h - this->center_wall_detect_d, center_h + this->center_wall_detect_d));
    ...
    ROS_INFO("[%s] Left wall: %f, Right wall: %f",this->drive_mode.c_str(), right_avg, left_avg );
...
void WallFollow::handle_slow_down(){
    ...
    if (this->turn_direction == "right" && center_avg > this->slow_threshold) {
        this->drive_mode = "PID";
        ROS_INFO("(Mode Change) %s -> PID (Deep Detected? %d)", this->drive_mode.c_str(), this->deep_wall_ahead);
        this->drive_speed = this->normal_speed;
        drive(this->drive_speed);
        this->yaw_before_turn = this->yaw;
    }
